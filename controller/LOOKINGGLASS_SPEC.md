...
Download test files and test IPs are displayed inline with each node in the sidebar node selector panel, not on a separate dedicated page.

When a visitor selects a node, the sidebar expands to show:

- **Test IPs**: the node's IPv4 and IPv6 addresses (selectable for easy copy)
- **Download test files**: one-click download buttons for each available test file on that node

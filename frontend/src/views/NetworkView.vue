...
<template>
  <div class="mx-auto max-w-7xl px-4 py-8 sm:px-6">
    <div class="mb-8">
      <h1 class="text-2xl font-bold text-white sm:text-3xl">Network Information</h1>
    ...
    <div v-else-if="network" class="space-y-6">
      ...
      <div class="rounded-lg border border-gray-700 bg-gray-800/50 p-6">
        <h2 class="mb-4 text-lg font-semibold text-white">Network Overview</h2>
        <dl class="grid grid-cols-1 gap-4 sm:grid-cols-2">
          <div v-if="network.network_name">
            <dt class="text-sm text-gray-500">Network Name</dt>
            <dd class="mt-1 text-base font-medium text-white">{{ network.network_name }}</dd>
          </div>
          <div v-if="network.asn">
            <dt class="text-sm text-gray-500">ASN</dt>
            <dd class="mt-1 font-mono text-base font-medium text-white">AS{{ network.asn }}</dd>
          </div>
          <div v-if="network.peering_policy">
            <dt class="text-sm text-gray-500">Peering Policy</dt>
            <dd class="mt-1 text-base text-white">{{ network.peering_policy }}</dd>
          </div>
      ...
      <!-- IPv4 Prefixes -->
      <div v-if="network.ipv4_prefixes.length" class="rounded-lg border border-gray-700 bg-gray-800/50 p-6">
        <h2 class="mb-4 text-lg font-semibold text-white">IPv4 Prefixes</h2>
        <div class="space-y-1">
          <div
            v-for="(prefix, i) in network.ipv4_prefixes"
            :key="i"
            class="rounded bg-gray-800 px-3 py-1.5 font-mono text-sm text-gray-300"
          >
            {{ prefix }}
          </div>
        </div>
      ...
      <!-- IPv6 Prefixes -->
      <div v-if="network.ipv6_prefixes.length" class="rounded-lg border border-gray-700 bg-gray-800/50 p-6">
        <h2 class="mb-4 text-lg font-semibold text-white">IPv6 Prefixes</h2>
        <div class="space-y-1">
          <div
      ...
      <!-- Internet Exchanges -->
      <div v-if="network.internet_exchanges.length" class="rounded-lg border border-gray-700 bg-gray-800/50 p-6">
        <h2 class="mb-4 text-lg font-semibold text-white">Internet Exchanges</h2>
        <div class="space-y-3">
          <div
          ...
          >
            <span class="text-white">{{ ix.name }}</span>
            <a
              v-if="ix.url"
              :href="ix.url"
              target="_blank"
      ...
      <!-- Contact Information -->
      <div class="rounded-lg border border-gray-700 bg-gray-800/50 p-6">
        <h2 class="mb-4 text-lg font-semibold text-white">Contact</h2>
        <dl class="grid grid-cols-1 gap-4 sm:grid-cols-2">
          <div v-if="network.noc_contact">
            <dt class="text-sm text-gray-500">NOC Contact</dt>
            <dd class="mt-1 text-white">{{ network.noc_contact }}</dd>
          </div>
          <div v-if="network.abuse_contact">
            <dt class="text-sm text-gray-500">Abuse Contact</dt>
            <dd class="mt-1 text-white">{{ network.abuse_contact }}</dd>
          </div>
        </dl>
        <div
